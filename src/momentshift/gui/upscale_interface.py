...
# =============================================================================
def _scaled_report(report: ProgressCb, lo: int, hi: int):
    """把一个 0~100 进度回调映射到 [lo, hi] 区间。

    放大阶段占前段、压缩阶段占后段，避免两阶段进度互相覆盖看着像卡死。
    """

    def _r(pct):
        try:
            report(lo + max(0, min(100, int(pct))) * (hi - lo) / 100.0)
        except Exception:
            pass
