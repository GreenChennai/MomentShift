...
# ===========================================================================
# 品牌色 —— 绿系（语义分组之一，深色主题将按组整体映射）
# ===========================================================================
# UI 重构（2026-09）确立的分组契约：全部色彩按「品牌绿 / AI 青 / 功能状态 /
# 中性灰阶」四个语义组维护；将来接深色主题时按组映射暗色对，而不是逐个挑色。
...
ACCENT_EMPH = "#2DA44E"  # 主色强调档（GitHub success emph）：进度填充/就绪态等需要更亮一档的场合
...
# AI 功能色 —— 青系（语义分组之二）
# ===========================================================================
# 只给「AI 能力」表面用（放大/插帧、音频转文字的引擎状态、模型徽标、推理进度），
# 克制原则：做强调底色 / 徽标 / 进度填充，不做整页换色。
# 与品牌绿同属冷调、亮度对齐（对比 #1F883D），浅色底上白字对比度 ≥ 4.5:1。
AI_ACCENT = "#0E7C8C"  # AI 主青
AI_HOVER = "#0C6B79"  # AI 青悬停
AI_PRESS = "#0A5F6B"  # AI 青按下
AI_SOFT = _rgba(AI_ACCENT, "0.08")  # AI 淡底（徽标/就绪提示）
AI_SOFT_STRONG = _rgba(AI_ACCENT, "0.15")  # AI 选中态底
AI_TINT_TEXT = "#0A5E6B"  # AI 淡底上的深青文字（正文对比度达标）
...
# 圆角体系锁（2026-09 UI 重构裁决）：卡片 12（RADIUS/RADIUS_LG）、按钮与输入
# 8（RADIUS_MD）、徽标/小件 3-6、状态胶囊 9。全项目只用这一套，勿再发明新值。
...
FONT_TITLE = 18
FONT_TITLE_LG = 20  # 功能页大标题（2026-09 字阶：标题 20 / 卡题 13 粗 / 正文 13）
...
def soft_chip_qss(fg: str, bg: str, *, radius: int = 6, padding: str = "2px 9px") -> str:
    """淡底强调芯片（拖拽区格式徽标、压缩策略芯片）。

    与 :func:`pill_qss`（实底反白）相对：这里用「淡底 + 同系深字」，视觉更轻，
    适合一排并陈的静态信息，不与状态胶囊争夺注意力。
    """
    return (
        f"color: {fg}; background: {bg}; border-radius: {radius}px;"
...
def ai_badge_qss(*, size: int = FONT_MICRO, padding: str = "1px 6px") -> str:
    """「AI」小徽标：青系淡底 + 深青粗体，标注 AI 能力表面（放大 / ASR 卡片）。"""
    return (
        f"color: {AI_TINT_TEXT}; background: {AI_SOFT_STRONG};"
        f" border-radius: {RADIUS_XS}px; padding: {padding};"
