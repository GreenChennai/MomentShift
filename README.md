...
- **转换进度与大小对比**：实时进度条 + 百分比，转换完成后对比前后文件大小，结果路径可一键复制。
- **智能格式选择**：单文件拖入即弹出「同类型」格式选择（图片→图片，视频→视频）；批量转换在开始弹窗中按图片 / 音频 / 视频分别指定目标格式。
...
### v0.2.6
- **仓库清理**：移除此前被误推到 GitHub 的构建目录 `dist_v021` / `dist_v022`（已通过 `.gitignore` 的 `dist_v0*` 规则忽略）；构建产物从此统一输出到仓库外的 `../构建/`，不再污染源码仓库（#1）。
- **启动速度排查**：实测 v0.2.5 的惰性加载与 v0.2.4 无异，瓶颈在 Windows 启动 `MomentShift.exe` 之后、界面真正开始加载之前。已关闭 PyInstaller 的 UPX 压缩（`upx=False`），避免 PyQt6 动态库被压缩后触发 Windows Defender 重复扫描与内存解压，从而加快冷启动；其余耗时主要来自 PyQt6 + qfluentwidgets 运行时导入，惰性构建已尽量压到最低（如暂无明显更佳方案，先保持现状）（#2）。
- **修复「添加文件 / 媒体 / 图片」弹框重复弹出**：将文件选择框的打开从鼠标释放事件中延迟到下一事件循环（`QTimer.singleShot`），避免 Windows 在关闭原生对话框后重放释放事件导致再次弹框；无论选择还是取消均只弹一次（#3）。
- **「添加文件夹」按钮改为绿色**：与「加入队列」按钮一致，使用全局强调色 #238636 的实心绿底（#4）。
- **胶囊（图片 / 音频 / 视频等）配色**：格式胶囊背景改为绿色 #238636、白色文字；「或点击选择文件 / 文件夹」文案改为黑色 #212121（#5）。
- **修复圆角裁切与圆形蒙版溢出**：移除与遮罩冲突的阴影效果，对外层圆角容器做严格圆角裁切（`setMask` 圆角矩形），对中间圆形图标徽章做椭圆裁切（`setMask` 椭圆），消除四角白边与矩形色块，保证圆角闭合、圆形背景严格局限在圆形范围内（#6）。
- **「并发线程数」改为纯数字输入框**：移除原有的增加 / 减少步进按钮，仅保留数字输入框（#7）。
- ffmpeg 仍不内置，需用户自行放置 `ffmpeg.exe` / `ffprobe.exe` 到软件根目录。

### v0.2.5
- **启动极速优化**：所有界面（含首页 Convert）延后到首帧绘制之后再构建，且各界面重型依赖改为延迟导入；窗口与启动闪屏瞬时出现，打开软件接近「秒开」。
- **主题 / 语言切换**：主题切换改为「重启软件」方式即时生效，彻底修复浅色 ↔ 深色切换时设置页背景不跟随的问题（#5）；语言切换保持即时（进程内重翻，无重启）（#2）。
- **窗口尺寸**：调整为 **525 × 1000** 的竖向比例。
- **主题色统一**：全局强调色由旧蓝 / 青色系统一替换为 GitHub 绿 **#238636**（主按钮、开关、选中态、标题下划线等），替换了原有 `#009FAA` / `#29F1FF` / `#00A7B3` / `#69B3FC` 等旧色。
- ffmpeg 仍不内置，需用户自行放置 `ffmpeg.exe` / `ffprobe.exe` 到软件根目录。

> 更早版本的变更记录请见 Git 提交历史。
