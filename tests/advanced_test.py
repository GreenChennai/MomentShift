...
t = Task(id="x", input_path="/a/1.mp4", output_path="/a/out.mp4",
         target_format="mp4", category="video", use_gpu=False,
         merge=True, input_paths=["/a/1.mp4", "/a/2.mp4"])
args = build_args(t, {})
check(any("filter_complex" in a for a in args) and any("concat=n=2" in a for a in args),
...
t2 = Task(id="y", input_path="/a/1.mp4", output_path="/a/out.mp4",
          target_format="mp4", category="video", use_gpu=False)
args2 = build_args(t2, {})
check(any("scale=1280:720" in a and "fps=30" in a for a in args2),
      "video resolution + fps applied via -vf")
check(any(a == "-b:v" for a in args2) and any("5M" in a for a in args2),
      "video bitrate applied")

# 3) image quality (lossy jpg) --------------------------------------------
advanced.reset()
advanced.adv["image"]["quality"] = 50
t3 = Task(id="z", input_path="/a/1.png", output_path="/a/out.jpg",
...
advanced.reset()
advanced.adv["video"]["merge"] = True
added, skipped = mgr.add_files([p1, p2], "mp4", None, False,
                                output_mode="same", suffix="_c")
...
check("image" in ap._cat_panels and "video" in ap._cat_panels
      and "audio" in ap._cat_panels, "advanced panel builds per-category sub-panels")
