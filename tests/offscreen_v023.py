...
def main():
    ...
    from momentshift.gui.compress_interface import CompressWorker
    ...
    result = {}
    w = CompressWorker(
        item_id="c1", src=src, out=out, target_fmt="same",
        mode="lossless", quality=100, preferred="pillow", opts={},
    )
    w.signals.finished.connect(
        lambda i, ok, saved, detail: result.update(
            id=i, ok=ok, saved=saved, detail=detail))
    ...
    step(f"compress 'same' OK -> ok={result['ok']} saved={result['saved']} "
         f"detail={result['detail']!r}")

    # ------------------------------------------------------------------ #2 log
    step("logger: a compress log file must exist and contain the run line")
    logs = sorted(glob.glob(os.path.join(log_dir(), "*.log")))
    assert logs, f"no log files in {_LOG_DIR}"
    combined = "\n".join(open(lp, encoding="utf-8", errors="replace").read()
                          for lp in logs)
    ...
    class FakeCard:
        ...
        def isCollapsed(self): return self._c
    ...
    self._collapsibles = [
        FakeCard(True, False), FakeCard(True, False), FakeCard(True, False)]
